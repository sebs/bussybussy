...
export class JBFBusFactorCalculator extends EventEmitter {
  ...
  async calculateJBF(fileAuthorship, analysisData) {
    ...
    const fileCommitData = await this.getFileCommitHistory(analysisData.repoPath);
  ...
  async getFileCommitHistory(repoPath) {
  ...
  generateReport(analysisData, jbfResult) {
    ...
    const report = {
      ...
      topContributors: jbfResult.authorDetails.slice(0, 10).map(({ author, doa, recentContributions }) => ({
        author,
        weightedDegreeOfAuthorship: (doa * 100).toFixed(2) + '%',
