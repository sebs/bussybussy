...
export class JBFBusFactorCalculator extends EventEmitter {
  ...
  async getFileCommitHistory(repoPath, fileAuthorship) {
    ...
    try {
      // Get all commit data in one command - much more efficient than N+1 queries
      ...
      Object.keys(fileAuthorship).forEach(file => {
        fileCommitData[file] = [];
      });
      
      // Parse the output into a map structure
      ...
      for (const commitBlock of commits) {
        ...
        const [hash, timestamp, author] = lines[0].split('|');
        const timestampMs = parseInt(timestamp) * 1000;
        
        // Files changed in this commit (lines after the first)
        for (let i = 1; i < lines.length; i++) {
          const file = lines[i].trim();
          if (file && fileAuthorship[file]) {
            fileCommitData[file].push({
              author,
      ...
      // Sort commits by timestamp for each file (most recent first)
      for (const file of Object.keys(fileCommitData)) {
        fileCommitData[file].sort((a, b) => b.timestamp - a.timestamp);
      }
    } catch (error) {
      console.error('Error getting commit history:', error);
      // Initialize all files with empty arrays on error
      Object.keys(fileAuthorship).forEach(file => {
        fileCommitData[file] = [];
      });
    }
